...
set.seed(5)
pcr.mod <- pcr(recovery_time~.,data = train,
...
# Check number of components
set.seed(5)
pls.mod <- plsr(recovery_time~.,data = train,
                scale = TRUE, validation = "CV")
summary(pls.mod)
...
Based on the summary and plot, we would likely use the Principal Component Regression or Partial least squares models to predict our response because they minimize the mean RMSE over resamples.
